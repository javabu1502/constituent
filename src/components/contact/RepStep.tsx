...
export function RepStep({ state, dispatch, onBack }: RepStepProps) {
  ...
  // Issue-aware guidance: when the issue is already known (news/campaign
  // deep links prefill it), badge the officials who can actually act on it.
  // Users who need help choosing use the guided Assistant path instead.
  ...
  return (
    <div className="p-6 sm:p-8">
      ...
      {/* Deep-linked issue (from news or a campaign): offer the best-match shortcut */}
      {guidance && issue && (
        <div className="mb-5 p-3 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-xl flex items-center justify-between gap-3 flex-wrap">
          <p className="text-sm text-purple-900 dark:text-purple-200">
            Writing about <span className="font-semibold">{issue}</span> — we&rsquo;ve flagged the officials who can act on it.
          </p>
          <button
            onClick={handleSelectRecommended}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold rounded-lg transition-colors shrink-0"
          >
