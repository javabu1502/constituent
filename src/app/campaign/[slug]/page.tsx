...
export default async function CampaignPage({ params }: PageProps) {
  ...
  const { data, error } = await admin
    .from('campaigns')
    .select('id, slug, headline, description, issue_area, issue_subtopic, target_level, status, campaign_type, visibility, message_template, bill_level, bill_state, bill_ref, bill_title, bill_url, story_prompt, usage_statement, usage_tags, attribution_options, edit_revoke_policy, action_count, story_count, created_at')
    .eq('slug', slug)
    .eq('approval_status', 'approved')
    .single();
